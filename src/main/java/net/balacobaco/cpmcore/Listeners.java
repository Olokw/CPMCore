...
public class Listeners {
    public static void register() {
        try {
            ...
            for (ClassPath.ClassInfo classInfo : cp.getTopLevelClassesRecursive("net.balacobaco.cpmcore.eventos")) {
                Class<?> c = Class.forName(classInfo.getName());
                if (Listener.class.isAssignableFrom(c)) {
                    Listener listener = (Listener) c.getDeclaredConstructor().newInstance();
                    Bukkit.getServer().getPluginManager().registerEvents(listener, CPMCore.instance);
                }
            }
